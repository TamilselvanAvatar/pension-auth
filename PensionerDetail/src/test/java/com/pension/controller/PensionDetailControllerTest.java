...
class PensionDetailControllerTest {
	...
	void setUp() {
		...
		pensioner = DetailsOfPensioner.builder().aadhaar(987654321098l).allowances(2000).dob(new Date(2022, 5, 15))
				.name("ABC").PAN("BAJPC4350N").salaryEarned(15000).bankDetail(bankDetail).typeofPension(TypeOfPension.FAMILY)
				.build();
	}
	...
	void getPensionerByAadhaarTest1() throws Exception {
		...
		mockMvc.perform(
				MockMvcRequestBuilders.get("/PensionerDetailByAadhaar/987654321098").header("Authorization", ""))
	...
	void deletePensionerByAadhaarTest1() throws Exception {
		...
		mockMvc.perform(MockMvcRequestBuilders.delete("/DeletePensioner/987654321098").header("Authorization", ""))
				.andExpect(MockMvcResultMatchers.status().isOk());
	}
	...
	void savePensionerTest1() throws Exception {
		...
		mockMvc.perform(MockMvcRequestBuilders.post("/SavePensioner").header("Authorization", "").contentType(MediaType.APPLICATION_JSON_VALUE)
				.content("{\r\n" + "    \"name\": \"ABC\",\r\n" + "    \"dob\": \"1997-02-28\",\r\n"
	...
	@Test
	void savePensionerTest2() throws Exception {
		Mockito.when(service.insert(pensioner)).thenReturn("Saved");
		mockMvc.perform(MockMvcRequestBuilders.post("/SavePensioner").header("Authorization", "").contentType(MediaType.APPLICATION_JSON_VALUE)
				.content("{\"name\": \"ABC\",  \"dob\": \"2022-05-15\","
						+ " \"aadhaar\": 987654321098, \"salaryEarned\": 15000,"
						+ " \"allowances\": 2000, \"typeofPension\": \"FAMILY\","
						+ " \"bankDetail\": { \"name\": \"IOB\","
						+ "  \"accountNumber\": 89076543245, \"bankType\": \"PUBLIC\" "
						+ "},\"pan\": \"BAJPC4350N\" }"))
		//.andExpect(MockMvcResultMatchers.status().isOk())
